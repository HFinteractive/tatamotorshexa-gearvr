...
public class OVRPlayerController : MonoBehaviour
{
	...
	private bool fwdTrigger = false;
	private bool bckTrigger = false;
	...
	public virtual void UpdateMovement()
	{
		...
		if (ApplicationManager.Instance.isExprLaunched)
		{
			if (!CameraController.Instance.isInsideAuto)
			{
				...
				if (fwdTrigger)
				{
					VehicleController.Instance.HandleRotation (1);
				}
				if (bckTrigger)
				{
					VehicleController.Instance.HandleRotation (-1);
		...
		if (moveForward)
			MoveThrottle += ort * (transform.lossyScale.z * moveInfluence * Vector3.forward) * 3.0f;
		if (moveBack)
